...
import { chestLidBlocked } from '../src/skills.mjs'
let pass = 0, fail = 0
...
t('the deposit looks at the lid, releases sneak, and opens the chest under an 8-second budget with a named class', () => {
  ...
  const lid = f.indexOf('if (lid && chestLidBlocked(lid)) {'), open = f.indexOf('chest = await withTimeout(bot.openContainer(chestBlock), 8_000')
...
t('MUTANT: opening without the lid check is caught', () => {
  const c = strip(readFileSync(new URL('../src/skills.mjs', import.meta.url), 'utf8')); const anchor = 'if (lid && chestLidBlocked(lid)) {'
