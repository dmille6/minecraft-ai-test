...
t('A REFUSED PILLAR IS NOT SCORED AS A FAILED ESCAPE — but an EXHAUSTED one is', () => {
  const code = strip('../src/reflex.mjs')
  assert.match(code, /let climbed = null[\s\S]{0,160}climbed = await withinBody\(entombedGrant, \(\) => pillarOut\(bot[^)]*\)/,   // measured height (climbNeedAbove), run inside the entombed grant's async context (the actuator gate)
