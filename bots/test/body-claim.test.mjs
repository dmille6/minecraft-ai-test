...
t('THE CLIMB OPTS OUT: it wants the hole, not the cobble', () => {
  ...
  assert.equal(digs.length, 7, `expected 7 dig call sites, found ${digs.length}`)
  const optedOut = digs.filter(d => /needsDrop:\s*false/.test(d))
  assert.equal(optedOut.length, 6,
    'the two escape digs, the staircase re-dig, place()\'s make-room dig, goto\'s ' +
    'float dig and collectManually\'s open-above dig (the cell over a BURIED ore, so its ' +
    'drop can be reached; 2026-09-12) may opt out — all six want the HOLE, not the drop. The staircase one ' +
    ...
    'not dig while the bot is off the ground; with the watchdog on any of them would ' +
    'wait for cobble that unharvestable stone never yields.')
...
t('THERE IS NO UNTYPED READER: "is any skill running?" stays unaskable', () => {
  ...
  for (const r of reads) {
    assert.equal(r, 'bodyClaimFor', `reflex.mjs touches ${r}; only bodyClaimFor('climb'|'stair'|'dig') is allowed`)
  }
...
t('NO WATER PATH READS IT — checked globally, not in a window', () => {
  ...
  assert.deepEqual(calls.sort(), ['climb', 'dig', 'stair'],
    `expected exactly the three typed reads 'climb', 'dig' and 'stair' in reflex.mjs, found ${calls.length}: ${calls}`)
  ...
  assert.ok(line && /bodyClaimFor\?\.\('dig'\)/.test(line), "the 'dig' read must share the entombment guard line with 'climb'")
...
t("MUTANT: a 'swim' read inserted anywhere in reflex.mjs is still caught by the global guard", () => {
  ...
  assert.notDeepEqual(calls, ['climb', 'dig', 'stair'], 'the mutant must change the read set')
})
