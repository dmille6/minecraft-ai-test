...
t('WIRED: the dig loop runs canopyDrop first and refuses without digging; freed needs a solid block under the feet on ground (air is not ground)', () => {
  ...
  assert.ok(src.includes("if (descent && descent.refused) {") && src.indexOf("failClass: 'canopy_refused'") > src.indexOf("if (descent && descent.refused) {"), 'gather stops on a refused descent before any dig')
