...
await t('on a tie the bot does not turn for nothing', () => {
  ...
  assert.deepStrictEqual(c.bear, S, 'all four run the full depth, so keep facing')
})
...
await t('escapeBearings agrees with skills.mjs stairBearings in all four quadrants', () => {
  // The reflex keeps its own copy of the compass so it need not import a
  // 4,000-line skill module. This is the drift guard that makes that safe.
  for (let q = 0; q < 4; q++) {
    const yaw = q * (Math.PI / 2)
    assert.deepStrictEqual(escapeBearings(yaw), stairBearings({ entity: { yaw } }),
      `quadrant ${q}: the two rescues must not disagree about which way is ahead`)
  }
})
...
const UNEQUIP = "    if (bot.heldItem) await bot.unequip('hand').catch(() => {})"

await t('MUTANT KILLED: without the unequip the climb swings the last pickaxe', async () => {
  await withMutant(REFLEX_PATH, UNEQUIP, '', async mod => {
    const pick = { name: 'wooden_pickaxe', type: 101, count: 1 }
    const bot = makeBot(POCKET(), { inv: [pick], held: pick })
...
await t('WIRED: the ramp is tried before the scaffold prerequisite, not after it', () => {
  const call = REFLEX_CODE.indexOf('await escapeStairUp(bot)')
...
const RAMP_CALL = '            const ramp = await escapeStairUp(bot).catch(e => {'
...
await t('MUTANT KILLED: unwiring the call is caught, and caught for the right reason', async () => {
  ...
  const mutated = src.replace(RAMP_CALL, '            const ramp = ({ steps: 0, climbed: 0, stopped: "x" }) || (async e => {')
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .split('\n').filter(l => !l.trim().startsWith('//')).join('\n')
  assert.strictEqual(mutated.includes('await escapeStairUp(bot)'), false,
