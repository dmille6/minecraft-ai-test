...
import { headroomBreach, chooseStairUpBearing, stairUpStep } from '../src/scaffold.mjs'
import { escapeStairUp, refusalStreak, isEntombedForTest } from '../src/reflex.mjs'
...
await t('a LAVA ceiling is nothing to break, because lava is not a solid cell', () => {
  // Written down because the first version of `headroomBreach` refused here and
  // the refusal was unreachable: lava reports an EMPTY boundingBox, so the cell
  // has already answered "already open". A guard that cannot fire reads as
  // safety and is not. The bot is not left under it either -- the ramp's own
  // step checks lava on every cell it enters, and the step moves the bot out
  // from under this column rather than up into it.
  const w = TOMB(); w.set(0, 2, 0, 'lava')
  const r = headroomBreach({ at: atOf(w) })
  assert.strictEqual(r.ok, true, r.reason)
  assert.deepStrictEqual(r.dig, [], 'a liquid ceiling must not be dug at')
})
...
const ENTOMBED_CALL_EXPR = 'const stair = await escapeStairUp(bot)'
...
const ALREADY_OPEN = '  if (passable(over)) return { ok: true, dig: [] }'
const ALREADY_OPEN_REFUSES = "  if (passable(over)) return { ok: false, reason: 'headroom already open' }"
...
await t('MUTANT KILLED: unwiring the entombed call is caught, for the right reason', () => {
  ...
  const mutated = src.replace(ENTOMBED_CALL_EXPR, 'const stair = ({ steps: 0, stopped: "x" }) || (async () => {')
    .replace(/\/\*[\s\S]*?\*\//g, '')
    .split('\n').filter(l => !l.trim().startsWith('//')).join('\n')
