...
t('skills run inside their grant and the routed climbs inside theirs, so the actuator gate knows the caller', () => {
  ...
  assert.match(i, /if \(config\.reflex\.arbiter && runner\?\.arb\) \{[\s\S]{0,700}?runner\.arb\.installActuatorGate\(bot/, 'the gate is installed at spawn when the flag is on (the baseline names the refusal call site first)')
})
