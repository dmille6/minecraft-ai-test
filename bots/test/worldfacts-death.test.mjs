...
try {
  ...
  t('a second death of the same class within 8 blocks merges: count +4, deaths 2, both reporters; another class is another site', () => {
    const s = wf.reportDeath('fire', { x: 338, y: 36, z: 210 }, 'placebo-b-Delta')
  ...
  t('decay halves once per 12-h period: a 13-h-old site survives two consecutive writes at half, not a quarter', () => {
    ...
    assert.ok(fire().decayedAt > Date.now() - 60_000, 'the halving is timestamped')
  })
