...
import { harvestSafe, stairUpStep, chooseStairUpBearing, headroomBreach } from './scaffold.mjs'
    ...
    try {
      ...
      if (!escaping && !marooned && !climbing && isEntombed(bot) &&
        ...
        if (climbed === 'needs_blocks' || climbed === 'needs_pickaxe') {
          // SCOPED TO THIS HOLE. See `refusalStreak`: the counter gates a
          // rescue that digs, so four refusals in four different places must
          // not buy what four refusals in one pocket buys.
          climbRefusals = refusalStreak(climbRefusals, lastRefusalPos, bot.entity?.position)
          ...
          if (climbRefusals % ESCAPE_GIVE_UP_AFTER === 0) {
            ...
            const stair = await escapeStairUp(bot).catch(e => {
            ...
            if (stair.steps > 0) {
              ...
              climbRefusals = 0
              escapeFailures = 0
              logEvent({ kind: 'entombed_ramp_cut', status: 'success',
                         detail: `pillar declined (${climbed}) at y=${Math.round(yBefore)} for want ` +
                                 `of blocks; broke ${stair.breached} ceiling cell(s) and cut ` +
                                 `${stair.steps} bare-handed stair step(s), climbing ` +
                                 `${stair.climbed.toFixed(1)} — stopped because ${stair.stopped}`,
                         snapshot: snapshot(bot) })
            } else {
              const want = climbPrereqFor(climbed)
              bot.pendingPrereq = { ...want,
                because: `the climb declined ${climbRefusals}x (${climbed}) at ` +
                         `y=${Math.round(bot.entity.position.y)} and no escape ramp could ` +
                         `be cut (${stair.stopped})` }
              logEvent({ kind: climbed === 'needs_pickaxe' ? 'entombed_needs_pickaxe'
                         ...
                         detail: `pillar declined ${climbRefusals}x (${climbed}) at ` +
                                 `y=${Math.round(bot.entity.position.y)}; the escape ramp ` +
                                 `stopped because ${stair.stopped}; asked the goal layer ` +
...
export async function escapeStairUp (bot, {
  ...
  // than a refusal, so the maroon branch runs exactly as it did.
  {
    ...
    if (!plan.ok) {
      ...
      return { steps: 0, climbed: 0, stopped: plan.reason, bearing: null, runway: 0, breached: 0 }
    }
    ...
    if (plan.dig.length && bot.heldItem) await bot.unequip('hand').catch(() => {})
    for (const [dx, dy, dz] of plan.dig) {
      const b = bot.blockAt(p.offset(dx, dy, dz))
      if (!b) { stopped = 'terrain not loaded'; break }
      const budget = planDig(predictedDigMs(b, null))
      if (budget.refuse) { stopped = `cannot clear ${b.name} overhead by hand`; break }
      try { await digBounded(bot, b, budget.budgetMs) } catch (e) {
        stopped = `ceiling dig failed on ${b.name}: ${e.message}`
        break
      }
      breached++
    }
    if (stopped) {
      bot.clearControlStates()
      return { steps: 0, climbed: bot.entity.position.y - startY, stopped, bearing: null, runway: 0, breached }
    }
    // GRAVEL SETTLES INTO THE CELL YOU JUST OPENED. The loop below re-plans from
    // the world as it is, so a refilled ceiling reports itself as `no headroom`
    // rather than being silently climbed into.
    if (breached) await sleep(200)
  }
