...
if not R or len(kinds) < 10 or len(bots) < 2:
    ...
    sys.exit(1)
# BOT-HOURS ARE OBSERVED AND CLAMPED TO THE WINDOW, NOT ASSUMED.
#
# This was `len(bots) * HOURS`, which credits every bot that appeared ANYWHERE in
# the window with the WHOLE window. Every one of the five committed numbers is a
# rate with this on the bottom, so the error moves all of them together.
#
# THE OBVIOUS FIX IS ALSO WRONG, and it was measured rather than reasoned about.
# Counting distinct (bot, clock-hour) cells -- the shape seedread.py uses -- makes
# a 6 h window read 560 bot-hours for 80 bots instead of 480, a +16.7% OVERSTATEMENT,
# because the partial hours at each end count as whole cells. That is the opposite
# of the error being fixed, and at 24 h it is still +4%.
#
# So each cell is CLAMPED to its overlap with the window. A bot present through the
# whole window contributes exactly HOURS; a bot down for half of it contributes
# half; and the edges contribute only the minutes that are actually inside. The
# residual assumption left is that a bot with at least one row in an hour was up for
# that whole hour-slice -- silence inside an hour still reads as presence, which
# over-credits a bot that crashed mid-hour by at most that hour's slice.
import datetime as _dt
_end = max(r['t'] for r in R)
_start = _end - _dt.timedelta(hours=HOURS)
_cells = {((r['bot'] or {}).get('name'),
           r['t'].replace(minute=0, second=0, microsecond=0))
          for r in R if (r['bot'] or {}).get('name') and r['t'] >= _start}
bot_h = 0.0
for _b, _h0 in _cells:
    _h1 = _h0 + _dt.timedelta(hours=1)
    _ov = (min(_h1, _end) - max(_h0, _start)).total_seconds() / 3600.0
    if _ov > 0:
        bot_h += _ov
_assumed = len(bots) * HOURS
print(f"  bot-hours OBSERVED {bot_h:.1f} (bot x clock-hour cells, clamped to the window) "
      f"vs {_assumed:.0f} assumed ({100.0*(bot_h-_assumed)/_assumed:+.1f}%)")
...
print(block(R, "fleet", len(bots), bot_h / max(1, len(bots))))
