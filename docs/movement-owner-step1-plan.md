...
- 18 Sep 02:40Z — Codex pass 1 (REVISE FIRST, 8 findings) FOLDED: (1) rungs run inside `arb.within(grant, ...)`; (2) the gate attributes the pathfinder's tick by running the bot's physicsTick emission under a TICK marker, and every other contextless mover is refused while the body is held (`mayAct(ctx, holder, bound, tick)`; tests: arbiter-tick.test.mjs, arbiter.test.mjs's tick helper); (3) the episode deadline runs the arbiter's own stop then release, and the owner's wait is bounded (deadline + 5 s), so a placement that never settles cannot hold the owner or `escaping`; (4) adapter outcomes are judged by what happened (pillar: height gained; stair: steps, yielded ⇒ preempted; a revoked grant ⇒ preempted whatever the rung said); (5) bare-hand stairs are legal again, `blocksLeft` is enforced before the pillar, the stair gets the remaining deadline as its budget, and a hold after a refused pillar publishes `bot.pendingPrereq` via climbPrereqFor with an `entombed_needs_*` row; (6) episodes are keyed to their ORIGIN (a sub-threshold move keeps the episode; a relocation opens a new one), the latch counts HOLDS not openings, and closure is judged from the origin; (7) the route feeds `owner.evidence()` every tick a hold exists (displacement, block count, a pickaxe arriving, no longer entombed) and evidence resets the episode; the backoff is the arm's own 15-s interval; (8) the two `clearControlStates` the gate commits added on the air-release paths now run only under the arbiter, so OWNER=0 ARBITER=0 is byte-for-byte the old behaviour. Corpus: two SYNTHETIC entombed fixtures (sandbox/make-entombed-fixture.py: a sealed 1x1 pocket under a 3-thick stone cap, with and without a pickaxe) join Delta and Bravo in sandbox/corpus-owner.tsv; the earlier run had no entombment in it at all.
