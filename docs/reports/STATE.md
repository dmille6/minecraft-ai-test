...
_updated 2026-09-16 22:30 UTC_
...
- 22:25Z corrections: the loop's done-check was unscoped (matched the 1011b run's reads and skipped all of this run's); fixed in ~/canary-loop.sh and the loop restarted (pid changes; `pgrep -af canary-loop`), the +30 read ran late at 22:25Z. The registration's copied `control_rung_rows <= 0` own line (the rung is fleet-wide now) was removed before that read; own lines are deathread control_own_rows, pocketread blocks_p90, deathread nopath_per_bh_canary.
