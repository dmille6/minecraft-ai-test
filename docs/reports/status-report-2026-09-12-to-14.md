...
## Status 2026-09-16 21:30 UTC — the evening pass: what kills now, two instruments that lied, and the -13 bundle under the loop
**Fleet.** 80/80 online by RCON on all sixteen servers, TPS 20.0, one version (1d6c97d) since the 18:13Z promotion; no open loop; STATE.md current.
**Deaths, honestly.** 96 h: 416 on 7,680 bot-h (lava 198, drown 120, fall 96). After the 08a3da2 promotion (05:35Z, 1,216 bot-h): 31 = 0.026/bot-h — lava 5, drown 15, fall 11. A whole-fleet before/after, so "lava down 7x" is likely, not proven; the 0809 canary's own DiD (0 vs 9 lava deaths) points the same way. The residue is drowning first.
**Mechanisms (docs/reports/deaths-review-2026-09-16.md).** Every world has the same seed; four lava pools killed 46 bots in 48 h across eleven worlds, and 101 of 198 lava deaths in 96 h repeated a same-class death within 8 blocks in the same pool — hazards were on file and read only as prompt lines. Drownings: 47/50 idle, ~100 s of life after the first rescue row, and the new rung tried once per ten minutes, refusing bare hands before counting the digs. Falls: explore, 35 blocks median, not site-bound; the pathfinder's unlimited drop onto water is the hypothesis (an open upstream issue says the same), unproven — an analysis item.
**Reviews.** Codex pass 1, an independent Claude review with a prior-art search, Codex pass 2 adversarial. Both engines ranked the pocket-rung remedies first and the death-site exclusion second and named the same defects: the step price must stay under 100 across six chargings per diagonal (it is 16), death sites need their own weight and a once-per-period decay (the old prune halved on every write), a tool-free plan must never dig (ceiling and notch both guarded), a refusal must not spend the cooldown (only a granted body does), and the non-pathfinder mover — explore's fallback walk, the one that killed 5 of 5 after a corridor refusal — must consult the same list (it does, and a bot inside a disc may still walk out). The reduced-margin block plan was dropped by both.
**Built and shipped to the loop.** recovery-ladder-13 = b1659c0 (two commits on 1d6c97d; suite 188/188; branch pushed). Registration v17; RULE.md synced; deathread.py (repeat-site deaths DiD, death_site rows, noPath/bh) beside pocketread. Sandbox smoke (corpus 20260916T2109, ledge-over-lava with a seeded death site, goal inside the disc): candidate and control both alive, candidate moved 14 vs 11 and logged five route-crossing rows -- the price is live and does not strand a bot; avoidance is read on the canary. The host had no drawrec.sh for the loop's draw phase (the 1011b run was --no-act and skipped it); installed 21:05Z. The loop started 21:12Z; its first draw at ±25% found one eligible pool (the 5080-half median moved 45 -> 48 -> 38 items/bh within twenty minutes), so it redraws every 20 min and deploys when two pools qualify. drawrec.sh widened only when ±25% found NONE; fixed to widen when fewer than two qualify. 21:33:47Z: drawn placebo-a,placebo-b; 21:36Z VERIFIED split (b1659c0 on ten bots vs 1d6c97d on seventy); first read 22:04Z, verdict window +360 = 03:34Z, extension to +720 = 09:34Z.
**Instruments.** fleet-status.sh pointed at instance #1's retired hosts; fleet-doctor.py printed "in world 0/80   all present" off the worlds host; Events.rate(bots='auto') crashed on dict bots. All three fixed and tested (docs branch d775841); RCON per world is now the fleet-status server line.
**Not done.** Pooling (-12) not started; falls not built; the pocket-rung block floor (need 8, have 5) still open; no sandbox fixture exercises a tool-free water column.

## Status 2026-09-16 23:50 UTC — -13 reverted by its own instrument; re-registered as -13b; ELK restored
**The revert.** At 23:20Z the death poll reverted recovery-ladder-13 on one canary death (placebo-a-Delta, drowned at the surface in open water; 0.06/bh on 17 canary bot-h vs control 0.053). The link it found was `death_site_recorded` -- the row the death handler writes at the death -- which I had registered as a change row. A consequence row cannot be a linkage row; the owner's two-death floor would otherwise have applied. Torn down cleanly by the loop (three steps, ten bots back on 1d6c97d, ledger REVERT). Re-registered as -13b (v18) with only rows that show the change acted on the bot; the loop is drawing again (placebo pools excluded 12 h).
**Also found on the loop's first real run:** its done-check matched the previous run's reads and skipped every read of this run (fixed), and a copied own line would have falsely reverted at +30 (removed). The loop is now proven through draw, deploy, read, poll, revert and teardown, which is what the first real run was for.
**ELK.** The fleet's Elasticsearch had an expired trial license since 4 Sep; part of every filebeat batch has been dropped since 5 Sep and Kibana was unusable. Access to the VM restored from the mini (via pve1), basic license started, read-only `agent2` account created for the second agent; ingest resumed for skill and llm documents at 23:52Z once `bot.tools` and `llm.args_cleaned` were declared in the strict mappings (349 + 51 of 400 sampled documents had been rejected since 5 Sep). Files on .31 remain complete for the gap.
