...
## Status 2026-09-16 23:58 UTC — -13 reverted by its own instrument; re-registered as -13b; ELK restored
...
**ELK.** The fleet's Elasticsearch had an expired trial license since 4 Sep; part of every filebeat batch has been dropped since 5 Sep and Kibana was unusable. Access to the VM restored from the mini (via pve1), basic license started, read-only `agent2` account created for the second agent; ingest resumed 23:45Z. Files on .31 remain complete for the gap.
