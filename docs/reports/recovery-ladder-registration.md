...
## v12 as applied to recovery-ladder-08c = the LAVA GUARDS v2 + guard 1b (recovery-ladder-lava 3fb3548 = 3810457 + the fallback-walk guard; one Codex pass on the small patch), drafted 10:45Z; deployed after -08b's verdict
Change over -08b: explore's failed-leg fallback walk (the path a corridor refusal lands on) is judged like a route before it moves (seven blocks along the heading, prismarine's yaw convention), turns the other way when refused, stays put when both are refused (`explore_blind_step_refused` row), and no longer holds jump.
